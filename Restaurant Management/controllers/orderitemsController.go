...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error){
	...
	groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{order_id", "$order_id"},{"table_id", "$table_id"},{"table_number", "$table_number"}}},{"payment_due", bson.D{{"$sum", "$amount}}},{"total_count", bson.D{{"$sum", 1}}}},{"order_item",bson.D{{"$push", 1}}}
	...
	projectStage2 := bson.D{
		...
		if err = result.Al(ctx, &OrderItems); err != nil{
			panic(err)
		}
		defer cancel()
...
func GetOrderItem() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		err := orderItemCollection.FineOne(ctx, bson.M{"orderItem_id": orderItemId}).Decode(&orderItem)
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error":"error occured while listing ordered items"})
			return
	...
	return func(c *gin.Context){
		...
		for _, orderItem := range orderItemPack.Order_items{
			...
			if validationErr != nil{
				c.JSON(http.StatusBadRequest, gin.HandlerFunc["error": validationErr.Error()])
				return
			}
